      ...
      <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_320px]">
          <div className="space-y-8">
            <div className="overflow-hidden rounded-[36px] border border-border bg-card p-6 backdrop-blur-xl sm:p-12">
              ...
              <div className="space-y-5 p-6 lg:p-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {post.tags.map((tag) => (
                    <span key={tag} className="rounded-full border border-border bg-muted/20 px-3 py-1 text-xs text-muted-foreground">
                      {tag}
                    </span>
                  ))}
                </div>
            ...
            <article className="rounded-[36px] border border-border bg-card p-6 backdrop-blur-xl lg:p-8">
              ...
              <div className="mt-10 flex flex-wrap items-center gap-3 border-t border-border pt-6">
                ...
                <Link
                  ...
                  target="_blank"
                  className="rounded-full border border-border bg-muted/20 px-4 py-2 text-sm text-muted-foreground transition hover:bg-muted/40 hover:text-foreground"
                >
                  <XIcon />
                  X / Twitter
                </Link>
            ...
            <section className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-[30px] border border-border bg-card p-5 backdrop-blur-xl">
                <p className="text-xs uppercase tracking-[0.24em] text-muted-foreground">Previous</p>
                {previous ? (
                  <Link href={`/blog/${previous.slug}`} className="mt-3 block text-lg font-medium text-foreground transition hover:text-primary">
                    <ArrowLeft className="mr-2 inline size-4" />
                    {previous.title}
                  </Link>
                ) : (
                  <p className="mt-3 text-sm text-muted-foreground">No previous post</p>
                )}
              </div>
              <div className="rounded-[30px] border border-border bg-card p-5 text-right backdrop-blur-xl">
                <p className="text-xs uppercase tracking-[0.24em] text-muted-foreground">Next</p>
                {next ? (
                  <Link href={`/blog/${next.slug}`} className="mt-3 block text-lg font-medium text-foreground transition hover:text-primary">
                    {next.title}
                    <ArrowRight className="ml-2 inline size-4" />
                  </Link>
                ) : (
                  <p className="mt-3 text-sm text-muted-foreground">No next post</p>
                )}
              </div>
            </section>

            <section className="rounded-[36px] border border-border bg-card p-6 backdrop-blur-xl">
              <h2 className="text-2xl font-semibold text-foreground">Related Articles</h2>
              <div className="mt-5 grid gap-4 md:grid-cols-3">
                {relatedPosts.map((related) => (
                  <Link key={related.slug} href={`/blog/${related.slug}`} className="rounded-[24px] border border-border bg-muted/20 p-4 transition hover:bg-muted/30">
            ...
            <div className="rounded-[30px] border border-border bg-card p-5 backdrop-blur-xl">
              ...
              <div className="mt-4 space-y-3">
                <input
                  ...
                  className="w-full rounded-2xl border border-border bg-muted/20 px-4 py-3 text-sm text-foreground outline-none placeholder:text-muted-foreground/50"
                />
            ...
            <div className="rounded-[30px] border border-border bg-card p-5 backdrop-blur-xl">
              ...
              <div className="mt-4 space-y-3 text-sm text-muted-foreground">
                <p className="flex items-center gap-2">
                  <CalendarDays className="size-4 text-muted-foreground/50" /> Published {formatDate(post.date)}
                </p>
