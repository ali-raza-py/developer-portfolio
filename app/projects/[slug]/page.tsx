...
import { ArrowUpRight, CalendarDays, Github, Layers3, Sparkles } from 'lucide-react'
          ...
          <article className="rounded-[36px] border border-white/10 bg-white/[0.03] p-6 backdrop-blur-xl lg:p-8">
            <div className="mb-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              {project.metrics?.map((metric) => (
                <div key={metric.label} className="rounded-3xl border border-white/10 bg-black/30 p-4">
            ...
            <section className="mt-10 rounded-[30px] border border-dashed border-white/15 bg-white/[0.03] p-6">
              <div className="mb-4 flex items-center gap-2 text-sm uppercase tracking-[0.24em] text-white/45">
                <Layers3 className="size-4" />
                Architecture diagram placeholder
              </div>
              <div className="grid min-h-52 place-items-center rounded-[24px] border border-white/10 bg-black/30 text-center text-white/45">
            ...
            {project.gallery.length > 0 && (
              <section className="mt-10">
                <h2 className="text-2xl font-semibold text-white">Screenshots Gallery</h2>
                <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
                  {project.gallery.map((image) => (
                    <div key={image} className="overflow-hidden rounded-[24px] border border-white/10">
          ...
          <aside className="space-y-6">
            <div className="rounded-[30px] border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
              ...
              <div className="mt-4 space-y-3">
                <Link href={project.liveUrl} target="_blank" className="flex items-center justify-between rounded-2xl border border-[#22C55E]/30 bg-[#22C55E]/10 px-4 py-3 text-sm text-[#D9FBE0] transition hover:bg-[#22C55E]/20">
                  <span>Live Demo</span>
                  <ArrowUpRight className="size-4" />
                </Link>
                <Link href={project.githubUrl} target="_blank" className="flex items-center justify-between rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm text-white/75 transition hover:bg-white/[0.05] hover:text-white">
            ...
            <div className="rounded-[30px] border border-white/10 bg-white/[0.04] p-5 backdrop-blur-xl">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {project.techStack.map((tech) => (
                  <span key={tech} className="rounded-full border border-white/10 bg-black/30 px-3 py-1 text-xs text-white/70">
                    {tech}
                  </span>
                ))}
              </div>
